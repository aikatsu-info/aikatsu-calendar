...
- 2026-09-14以降、**X @aikatsu_dcd（アイカツ！アンコール公式）のHP更新通知ポストから追加する場合**、`src`・`url` は投稿（X上のポスト）ではなく **公式サイト https://dcd.aikatsu.com/encore/** を指すようにする（`src:"アイカツ！アンコール公式サイト"`, `url:"https://dcd.aikatsu.com/encore/"`）。HP更新の実体は公式サイト側にあり、ポストはその告知に過ぎないため。

## url フィールドの有効性確認ルール（重要 — リンク切れ再発防止）

2026-09-17に、スクリーンショット中で途中まで見えていたURL（`aikatsu.net/portal/stars10/`）を推測で補完して登録した結果、実際の正しいパスは `stars10th/` で404になる事故が発生した。再発防止のため、新規エントリの `url` を確定する際は以下を守る。

1. **URLを推測・手打ちで作らない。** ユーザーが貼った完全なURL、スクリーンショット内で最初から最後まで読み取れているURL、公式サイトのリンクを辿って実際に確認できたURLだけを、一字一句そのままコピーして使う。「おそらくこのパスだろう」で末尾（`stars10/` のようなスラッグ）を補完するのは禁止。
2. **セッションで外部URLへのアクセス手段（`WebFetch`、`mcp__Claude_Browser__*` 等）が使える場合は、追加前に実際にそのURLを開いて中身が存在することを確認する。** `EGRESS_BLOCKED` 等でアクセス手段が使えない場合（頻発する）は、その旨を認識した上で1.の「推測しない」原則をより厳格に守る。
   - 例外: p-bandai.jp の商品ページ（`p-bandai.jp/item/item-xxxxxxx/`）のようにボット検知で恒常的にアクセス不可と分かっているドメインは、アクセス失敗＝リンク切れと判断しない（上記「情報源一覧」の既存の記載に従う）。
3. **スクリーンショット内でURLが途中で切れている場合**（例: `aikatsu.net/portal/stars10…`）、末尾を補完しない。ユーザーに完全なURLを確認するか、同じ告知が載っている別の情報源（ポータルのトピックス一覧など）を辿って正しい完全URLを特定する。どちらもできない場合は、詳細ページへの直接リンクではなく、確実に存在すると確認できる上位ページ（例: ポータルのトップページ）を `url` に使う。
4. **リンク切れが判明した場合**、同じ告知から追加した他のentryも同じ誤ったURLを使っている可能性が高いので、`grep` で同一URLを使っている全entryを洗い出し、`aikatsu_calendar.html` と `data/items.json` の両方をまとめて修正する。
